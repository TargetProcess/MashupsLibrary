tau.mashups
    .addDependency('libs/jquery/jquery').addMashup(function() {
		...
		/* change this to true if you only want to show efforts from the current sprint */
        ...
        function loadData() {
            $.getJSON(api_base+"Assignables?include=[Assignments[GeneralUser[FirstName,LastName,Role],Role],RoleEfforts[Effort,EffortToDo,TimeSpent,TimeRemain,Role],EntityType,EntityState,Name]&where=(EntityState.IsFinal%20eq%20'false')%20and%20(Effort%20gt%200)"+(SHOW_ONLY_CURRENT_ITERATION?"%20and%20(Iteration.IsCurrent%20eq%20'true')":"")+"&format=json&take=1000", formatResult);
        }
